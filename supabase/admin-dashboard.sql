...
-- A3. boost_purchases.status (repo schema has it; older live DB may not)
...
--     - Boost packages: 200 Cooptoken = $1, 310 = $2.50, 600 = $5
--       (boostPct/durationDays kept so the main app boost UI keeps working;
--        bonusCooptoken is the admin-visible reward figure)
--     NOTE: re-running this file resets these three values to the defaults above.
update public.admin_settings set
  points_per_coop = 1000.0000,
  base_mining_rate = 50.0000,
  daily_mining_hours = 12.00,
  boost_tiers = '[
    {"id":"starter","name":"Starter","priceUsd":1.00,"boostPct":25,"durationDays":7,"bonusCooptoken":200},
    {"id":"plus","name":"Plus","priceUsd":2.50,"boostPct":50,"durationDays":7,"bonusCooptoken":310},
    {"id":"pro","name":"Pro","priceUsd":5.00,"boostPct":75,"durationDays":7,"bonusCooptoken":600}
  ]'::jsonb
where id = 'default';
  ...
  return jsonb_build_object(
    ...
    'purchases', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'bonus_reward', b.bonus_reward, 'status', b.status, 'created_at', b.created_at
...
-- Adjusts a wallet's COOP or Cooptoken balance and records:
--   1. an 'admin' transaction visible in the user's wallet history
